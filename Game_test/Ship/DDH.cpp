...
DDH::DDH(float newx, float newy, float newr, float newangle, float newrm, float newanglem, int Type, bool Dir) : Trash(newx,newy,newr,newangle,newrm,newanglem,Type)
{
	Atk = 1;
	Def = 0;
...
{
	if (Direct)
		ship->SetTexture(table->getTexture(5));
		//ship->SetTexture(hge->Texture_Load("DDH-r.png"));
	else
		ship->SetTexture(table->getTexture(6));
		//ship->SetTexture(hge->Texture_Load("DDH-l.png"));
	...
	snd=table->getSound(5);
}

void DDH::damage(int ATK)
{
