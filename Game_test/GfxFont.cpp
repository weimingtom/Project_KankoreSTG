...
void GfxFont::Print( float x, float y, const char *format, ... )
{
	...
	static WCHAR wsBuffer[1024];


	char *lpsArg=(char*)&format+sizeof(format);
	vsprintf_s(sBuffer, format, lpsArg);

	MultiByteToWideChar( CP_ACP, 0, sBuffer,
        strlen(sBuffer)+1, wsBuffer,   
		sizeof(wsBuffer)/sizeof(wsBuffer[0]));

	Render(x,y,wsBuffer);
}
/*void GfxFont::wPrint( float x, float y, const wchar_t *format, ... )
{
	static WCHAR wsBuffer[1024];

	va_list args;
	va_start ( args, format );
	vswprintf_s(wsBuffer,1024, format, args);
	va_end ( args );
	Render(x,y,wsBuffer);
}*/
...
void GfxFont::Render(float x, float y, const wchar_t* text)
{
	...
	while(*text)
	{
		...
		if (*text == L'#'){
			++text;
				 if (*text == L'W'){m_pSprite->SetColor(ARGB(255,255,255,255));	++text;}//白
			else if (*text == L'K'){m_pSprite->SetColor(ARGB(255,0,0,0));		++text;}//黑
			else if (*text == L'R'){m_pSprite->SetColor(ARGB(255,255,0,0));		++text;}//红
			else if (*text == L'Y'){m_pSprite->SetColor(ARGB(255,255,255,0));	++text;}//黄
			else if (*text == L'B'){m_pSprite->SetColor(ARGB(255,0,0,255));		++text;}//蓝
			else if (*text == L'G'){m_pSprite->SetColor(ARGB(255,0,255,0));		++text;}//绿
			else if (*text == L'A'){m_pSprite->SetColor(ARGB(255,128,128,128));	++text;}//灰
			else if (*text == L'P'){m_pSprite->SetColor(ARGB(255,128,0,128));	++text;}//紫
			else if (*text == L'F'){m_pSprite->SetColor(ARGB(255,255,0,255));	++text;}//粉
			else --text;
		}
		//换行判定
		if (*text == L'\n' || *text == L'\r')
...
// 渲染文本扩展
void GfxFont::PrintEx( float x, float y, int number ,const char *format, ... )
{
	static char sBuffer[1024];
	static WCHAR wsBuffer[1024];
	...
	RenderEx(x,y,wsBuffer,number);
}

void GfxFont::RenderEx(float x, float y, const wchar_t* text ,int number)
{
	float offsetX = x;
	float offsetY = y;
	int text_Num = number;//最大显示多少字符
	m_pSprite->SetColor(ARGB(255,255,255,255));
